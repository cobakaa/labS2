...
void vector_print(const Vector * vector) {
    for (size_t i = 0; i < vector->size; ++i) {
        printf(" ");
        VItem tmp = get_VItem(vector, i);
        token_print(&tmp.item);
    }
    printf("\n");
}
...
Vector * vector_copy(Vector * l, Vector * r) {
    for (size_t i = 0; i < (vector_size(l) < vector_size(r) ? vector_size(l) : vector_size(r)); ++i) {
        l->data[i] = r->data[i];
    }

    if (vector_size(l) < vector_size(r)) {
        for (size_t i = vector_size(l); i < vector_size(r); ++i) {
